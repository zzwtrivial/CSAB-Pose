...
import natsort
import glob
from .u_util import opt_get
from . import options as option
from .u_util import get_resume_paths
import cv2
...
class LLFlowModel(BaseModel):
    def __init__(self, opt, step):
        ...
        self.netG = define_Flow(opt, step).to(self.device)
        #
    ...
    def optimize_parameters(self, step):
        ...
        if not self.already_print_params_num:
            logger.info("Parameters of full network %.4f and encoder %.4f" % (
                sum([m.numel() for m in self.netG.parameters() if m.grad is not None]) / 1e6,
                sum([m.numel() for m in self.netG.RRDB.parameters() if m.grad is not None]) / 1e6))
...
def get_model():
    opt = option.parse("/home/wjm/MyFinalProject/pytorch-cpn/networks/other/LLFlow/test.yml", is_train=False)
    opt['gpu_ids'] = None
    opt = option.dict_to_nonedict(opt)
    model = LLFlowModel(opt, step=0)

...
def test_4(model):
    # [1020, 830, 2789, 939]
    e_result = test(cfg_test_extreme, model, "Extreme", r"E:\Final\test-ll-extreme.lmdb")
    h_result = test(cfg_test_hard, model, "Hard", r"E:\Final\test-ll-hard.lmdb")
    n_result = test(cfg_test_normal, model, "Normal", r"E:\Final\test-ll-normal.lmdb")
    a_result = test(cfg_test_all, model, "All", r"E:\Final\test-ll-all.lmdb")

    # e_result = test(cfg_test_extreme, model, "Extreme", "../../test-ll-extreme-crop.lmdb")
    # h_result = test(cfg_test_hard, model, "Hard", "../../test-ll-hard-crop.lmdb")
    # n_result = test(cfg_test_normal, model, "Normal", "../../test-ll-normal-crop.lmdb")
    # a_result = test(cfg_test_all, model, "All", "../../test-ll-all-crop.lmdb")
    a = f"Extreme\t Test Acc: {e_result[0]}"
    b = f"Hard\t Test Acc: {h_result[0]}"
    c = f"Normal\t Test Acc: {n_result[0]}"
    d = f"All \t Test Acc: {a_result[0]}"
    print(a)
    print(b)
    print(c)
    print(d)
...
def create_model(m_type):
    ...
    elif m_type == "Chapter4_model":
        return network.__dict__[m_type](cfg.output_shape, cfg.num_class, pretrained=True)
    else:
        return network.__dict__[m_type](cpn_cfg0.output_shape, cpn_cfg0.num_class, False, pretrained=True)

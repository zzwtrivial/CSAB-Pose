...
def test(i_cfg, model, prefix, db_path):
    ...
    f_name = os.path.join("uni_test", f'll-{prefix}.json')
...
def test_4(model):
    # [1020, 830, 2789, 939]
    # e_result = test(cfg_test_extreme, model, "Extreme", "/ExLPose/test-ll-extreme.lmdb")
    # h_result = test(cfg_test_hard, model, "Hard", "/ExLPose/test-ll-hard.lmdb")
    # n_result = test(cfg_test_normal, model, "Normal", "/ExLPose/test-ll-normal.lmdb")
    # a_result = test(cfg_test_all, model, "All", "/ExLPose/test-ll-all.lmdb")
...
model_type = "CID_CPN50"
if __name__ == "__main__":
    model = create_model(model_type)
    model = torch.nn.DataParallel(model).cuda()

    model_path = "checkpoint/cid-10.pth.tar"

...
def main(args):
    ...
    model = network.__dict__[args.model](cfg.output_shape, cfg.num_class, args.freeze, pretrained=True,
                                         active = [args.active_light, args.active_denoise, args.active_fe])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_loader_all = torch.utils.data.DataLoader(
            EvalLLLmdb("/ExLPose/test.lmdb", cfg_test_all),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--active_light', action='store_false', help='默认开启，如果指定该参数则关闭')
    parser.add_argument('--active_denoise', action='store_false', help='默认开启，如果指定该参数则关闭')
    parser.add_argument('--active_fe', action='store_false', help='默认开启，如果指定该参数则关闭')

    parser.set_defaults(start_epoch=0, workers=12, num_gpus=1, epochs=15, checkpoint='ablation_ckpt')

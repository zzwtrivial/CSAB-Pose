...
class MyData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        os.path.join(self.img_folder, image_name)

        img_path_LL = os.path.join(self.img_folder, image_name)

        points = np.array(a['unit']['keypoints']).reshape(self.num_class, 3).astype(np.float32)
        np.array(a['unit']['keypoints']).reshape(self.num_class, 3).astype(np.float32)
        gt_bbox = a['unit']['GT_bbox']

        image_LL = imageio.v2.imread(img_path_LL)

        image_LL, points, _ = self.augmentationCropImage(image_LL, gt_bbox, points)
        return image_LL, points
...
if __name__ == "__main__":
    ...
    data = MyData(cfg)
    data2lmdb(train_loader, db_path="/home/wjm/MyFinalProject/ZeroShotDayNightDA/", name="WL")

...
class EvalLLData(data.Dataset):
    ...
    def wl_ll_path_pairs(self):
        pairs_file_path = r"E:\Final\Annotations\brightPath2darkPath.txt"
        wl_ll_pairs = {}
...
class EvalLLLmdb(data.Dataset):
    ...
    def wl_ll_path_pairs(self):
        pairs_file_path = r"E:\Final\Annotations\ExLPose\brightPath2darkPath.txt"
        wl_ll_pairs = {}
        with open(pairs_file_path) as f:
            x = f.readlines()
            for i in x:
    ...
    def __getitem__(self, idx):
        ...
        img, meta = pickle.loads(z)
        img = img.squeeze(0)
        meta['imgID'] = meta['imgID'].item()
        meta['det_scores'] = meta['det_scores'].item()
        meta['augmentation_details'] = meta['augmentation_details'].numpy().reshape(4, 1)
        return img, meta

...
class TrainingDataLL(data.Dataset):
    ...
    def augmentationCropImage(self, img_LL, bbox, joints=None):
        height, width = self.inp_res[0], self.inp_res[1]
        bbox = np.array(bbox).reshape(4, ).astype(np.float32)
        add = max(img_LL.shape[0], img_LL.shape[1])
        ...
        crop_size = min(crop_size, (bimg_LL.shape[1] - objcenter[0]) / width * min_shape * 2. - 1)
        crop_size = min(crop_size, objcenter[1] / height * min_shape * 2. - 1.)
        crop_size = min(crop_size, (bimg_LL.shape[0] - objcenter[1]) / height * min_shape * 2. - 1)
        ...
        if self.is_train:
            return img_LL, joints, details
        else:
            return img_LL, details
    ...
    def __getitem__(self, index):
        ...
        image_LL = imageio.v2.imread(img_path_LL)
        ...
        if self.is_train:
            image_LL, points, details = self.augmentationCropImage(image_LL, gt_bbox, points)
        else:
            image_LL, details = self.augmentationCropImage(image_LL, gt_bbox)

        if self.is_train:
            image_LL, points = self.data_augmentation(image_LL, points)
        ...
        if self.is_train:
            # save_param = [img_LL, img, targets, valid, meta]
            # torch.save(save_param, "/home/wjm/Dataset/ExLPose/PreBuild/pre_build_data_" + str(index))
            return img_LL, targets, valid
        else:
